...
export default function ProductConfigurator() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {/* Sign-in nudge */}
      {!loading && !user && (
        <div className="mb-6 flex flex-col gap-3 rounded-2xl border border-terracotta/30 bg-terracotta/5 p-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              {STEPS.map((s, i) => {
                ...
                return (
                  <li key={s.id} className={`flex items-center ${i < STEPS.length - 1 ? "flex-1" : ""}`}>
                    ...
                    >
                      <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-semibold transition-colors
                      ...
                      <span className={`hidden text-sm font-medium sm:block ${isActive || isDone ? "text-charcoal" : "text-muted"}`}>
                        {s.label}
                      </span>
                    </button>
                    {i < STEPS.length - 1 && (
                      <span className={`mx-3 h-0.5 flex-1 rounded-full transition-colors ${step > s.id ? "bg-terracotta" : "bg-line"}`} />
                    )}
                  </li>
                );
              })}
        ...
        <aside className="hidden lg:block">
          ...
          <div className="mt-4 rounded-2xl border border-line bg-cream/60 p-4 text-sm text-muted">
            ...
            <a href="https://wa.me/919084248821" target="_blank" rel="noopener noreferrer"
              className="mt-3 flex items-center justify-center gap-2 rounded-full bg-[#25D366] py-2.5 text-sm font-semibold text-white">
    ...
    <section>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {PRICING_TIERS.map((t) => (
          <button key={t.quantity} type="button" onClick={() => setQuantity(t.quantity)}
            ...
            {t.badge && (
              <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-terracotta px-2.5 py-0.5 text-[10px] font-semibold text-white">
                {t.badge}
              </span>
            )}
            <p className="text-2xl font-extrabold text-charcoal">{t.quantity}</p>
            <p className="text-xs text-muted">{t.quantity === 1 ? "magnet" : "magnets"}</p>
            <p className="mt-1.5 font-semibold text-terracotta">{CURRENCY_SYMBOL}{t.price}</p>
            <p className="text-[10px] text-muted">{CURRENCY_SYMBOL}{t.perPiece}/each</p>
          </button>
        ))}
      </div>
      <div className={`mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4 ${isCustom ? "border-terracotta bg-terracotta/10 ring-2 ring-terracotta/30" : "border-line bg-white"}`}>
        <div>
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setCustomQuantity((isCustom ? quantity : 12) - 1)}
            className="grid h-9 w-9 cursor-pointer place-items-center rounded-lg border border-line bg-white text-lg hover:bg-cream">−</button>
      ...
      <div className="mt-4 flex items-center justify-between rounded-xl border border-line bg-cream/60 px-4 py-3 text-sm">
        ...
        <span className="font-semibold text-terracotta">
          {CURRENCY_SYMBOL}{pricing.total} <span className="font-normal text-muted">(incl. 18% GST)</span>
        </span>
      </div>
    </section>
  );
    ...
    <section>
        ...
        {Array.from({ length: quantity }).map((_, i) => {
          ...
          if (url) return (
            <div key={i} className="group relative aspect-square overflow-hidden rounded-xl border-2 border-terracotta/30 bg-cream shadow-sm">
              ...
              <button type="button" onClick={() => removeImage(i)} aria-label={`Remove ${i + 1}`}
                className="absolute right-1 top-1 grid h-6 w-6 cursor-pointer place-items-center rounded-md bg-black/60 text-white hover:bg-red-600">
              ...
              {quantity > 1 && (
                <button type="button" onClick={() => useForAll(url)}
                  className="absolute inset-x-0 bottom-0 bg-black/65 py-1 text-[10px] font-medium text-white opacity-0 group-hover:opacity-100 cursor-pointer">
                  Use for all
                </button>
              )}
            </div>
          ...
          return (
            <button key={i} type="button" onClick={openPicker} disabled={uploading}
              className="grid aspect-square cursor-pointer place-items-center rounded-xl border-2 border-dashed border-line bg-white text-muted hover:border-terracotta/60 hover:text-terracotta disabled:opacity-60">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <button type="button" onClick={openPicker} disabled={uploading || remaining <= 0}
          className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-line bg-white px-4 py-2 text-xs font-semibold text-charcoal hover:bg-cream disabled:opacity-50">
          {uploading && <Spinner />}
        ...
        {images.length > 0 && quantity > 1 && (
          <button type="button" onClick={() => useForAll(images[0])}
            className="cursor-pointer rounded-full border border-line bg-white px-4 py-2 text-xs font-semibold text-charcoal hover:bg-cream">
            Use #1 for all {quantity}
          </button>
        )}
        {images.length > 0 && (
          <button type="button" onClick={clearAll}
            className="cursor-pointer text-xs text-muted hover:text-red-500">Clear all</button>
        )}
      </div>
  ...
  return (
    <section className="mt-16 border-t border-line pt-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="text-sm text-muted">Real photos from real orders — see the quality before you buy.</p>
      </div>

      <div className="mt-6 -mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2 sm:mx-0 sm:grid sm:grid-cols-2 sm:gap-5 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-2">
        {CUSTOMER_REVIEWS.map((r, i) => (
          <div key={i} className="w-[78%] shrink-0 snap-start overflow-hidden rounded-2xl border border-line bg-white shadow-sm sm:w-auto">
