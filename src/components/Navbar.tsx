...
const NAV_LINKS = [
  { href: "/", label: "Home" },
  { href: "/products/fridge-magnets", label: "Fridge Magnets" },
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-800/80 bg-[#050a18]/80 backdrop-blur-md">
        ...
        <div className="hidden items-center gap-3 md:flex">
          {!loading && user ? (
            <>
              ...
              <button
                onClick={() => logout()}
                className="rounded-full border border-slate-700 px-4 py-2 text-sm font-medium text-slate-200 transition-colors hover:bg-slate-800"
              >
                Sign out
              </button>
            </>
      ...
      {open && (
        <div className="border-t border-slate-800 bg-[#050a18] px-4 py-4 md:hidden">
            ...
            {!loading && user ? (
              <>
                ...
                <button
                  ...
                  className="rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-300 hover:bg-slate-800"
                >
                  Sign out
                </button>
              </>
              ...
              <>
                ...
                <Link
                  href="/products/fridge-magnets"
                  onClick={() => setOpen(false)}
                  className="mt-1 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 px-5 py-2 text-center text-sm font-semibold text-white"
                >
                  Order Now
                </Link>
              </>
